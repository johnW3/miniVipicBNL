...
int	      iLevel;
{
  ...
  // this worked
  //cards[card].base = (unsigned int *) mmap(0,255,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0x43C00000);
  // use a32base instead of hardcoding
...
//       cards[card].base = NULL;
//       epicsPrintf("devA32ZedConfig: vxMemProbe failed for Card %d",card);
//       return(ERROR);
//  }
...
struct longoutRecord *plo;
{
  ...
  if ( plo->out.value.vmeio.signal == 10 ) 
  {
	pixelToScanLO = plo->val;
	//plo->rbv = plo->val;
	return(0);
  }
  else if ( plo->out.value.vmeio.signal == 11 )
  {
	dacTrim = plo->val;
	return(0);
  }
  else if ( plo->out.value.vmeio.signal == 12 )
  {
	configPixelCnt = plo->val;
	return(0);
  }
  else if (write_card(plo->out.value.vmeio.card,pPvt->reg,
...
struct waveformRecord   *pwf;
{
  ...
  if (checkCard(pwf->inp.value.vmeio.card) == ERROR) return(ERROR);
   ...
   if( signal == 0 )
   {
      epicsPrintf("readWf signal = 0.  Scan pixel = %d (longout signal = 10), dacTrim = %d (longout signal = 11) \n", pixelToScanLO, dacTrim);
      scanPixelN( pixelToScanLO, dacTrim, configPixelCnt, pwf);
      //drvListBuckets( pvmeio->card, pRec->bptr, pRec->nelm, &num );
   }
   // scaling the x-axis for the threshold scan
   else if( signal == 1 )
   {
...
// SHOULD ONLY OPEN AND DO MMAP IN INIT ROUTINES
...
//   printf("FPGA Version : %d\n",fpgabase[3]); 
   unsigned long rawInValue = 0;
   read_card(pwf->inp.value.vmeio.card, 3, 0xffff, &rawInValue);
   printf("FPGA Version : %d\n", rawInValue);
   ...
   for (i=0;i<1024;i++) {
	   if (i == pixelToScan)
	   {
              ...
              CnfgPixel(pixelWordCnting, cards[pwf->inp.value.vmeio.card].base);
	   }
           else
              //CnfgPixel(pixelWordNoncnting,fpgabase);
	      CnfgPixel(pixelWordNoncnting, cards[pwf->inp.value.vmeio.card].base);
  ...
  //Pulse loadShadowReg
  //fpgabase[8] = 0x8;
  //write_card(pbo->inp.value.vmeio.card, pPvt->reg, pbo->mask, pbo->rval) 
  write_card(pwf->inp.value.vmeio.card, 8, 0xffff, 0x8);
  for (j=0;j<100;j++);
  //fpgabase[8] = 0x0;
  write_card(pwf->inp.value.vmeio.card, 8, 0xffff, 0x0);
  ...
  //fpgabase[52/4] = 1; 
  write_card(pwf->inp.value.vmeio.card, 13, 0xffff, 1);

  // global reset
  //issue a global reset to the miniVIPIC
  //fpgabase[GLOBRST] = 1;
  write_card(pwf->inp.value.vmeio.card, GLOBRST, 0xffff, 1);
  usleep(1);
  //fpgabase[GLOBRST] = 0;
  write_card(pwf->inp.value.vmeio.card, GLOBRST, 0xffff, 0);
  ...
  for (thn=thnStart;thn<=thnStop;thn=thn+thnStep) {
     ...
     TrigReadFrame(cards[pwf->inp.value.vmeio.card].base,frame2D,winTime);
